...
# 캐릭터 만들기 
character = pygame.image.load(os.path.join(image_path, "집게사장3.jpg"))
...
# 무기 만들기
weapon = pygame.image.load(os.path.join(image_path, "breadweapon1.png"))
...
# 사라질 무기, 공 정보 저장 변수. 
weapon_to_remove = -1   # 사라질무기
ball_to_remove = -1     # 사라질 공 
...
# 게임 종료 메시지 / TimeOver(시간초과), Misson Complete(성공), Game Over(캐릭터가 공에 맞았을때)
...
while running: 
    ...
    for ball_idx, ball_val in enumerate(balls):   
        ...
        # 공과 캐릭터 충돌 처리         
        if character_rect.colliderect(ball_rect): # 캐릭터가 공과 충돌하면 
            running = False # 중단된다. 
            break
        ...
        for weapon_idx, weapon_val in enumerate(weapons):
            ...
            if weapon_rect.colliderect(ball_rect):
                ...
                if ball_img_idx < 3:       
                    ...
                    # 나눠진 공 정보
                    small_ball_rect = ball_images[ball_img_idx + 1].get_rect()  # 현재 공보다 작아진 공의 정보. 
                    ...
                    balls.append({
                        ...
                        "to_x" : -3, # x 축 이동방향, -3 이면 왼쪽, 3 이면 오른쪽 
                        "to_y" : -6,    # y 축 이동방향,
                        "init_spd_y" : ball_speed_y[ball_img_idx + 1] }) # y 최초속도    튕길때마다 속도가 달라진다. 
                    # 오른쪽 
                    balls.append({ 
                        "pos_x" : ball_pos_x + (ball_width / 2) - (small_ball_width / 2),   # 공의 x 좌표
                        "pos_y" : ball_pos_y + (ball_height / 2) - (small_ball_height / 2),    # 공의 y 좌표
    ...
    # 충돌된 공 or 무기 없애기
    if ball_to_remove > -1:  # index 값이 0, 1, 2, 3 이기 때문이다. 
        del balls[ball_to_remove]   # ball_to_remove 값을 지운다. 
        ball_to_remove = -1         # 다음 프레임에 
    ...
    timer = game_font.render("Time : {} ".format(int(total_time - elapsed_time)), True, (255, 192, 203))
...
msg = game_font.render(game_result, True, (150, 75, 0))
